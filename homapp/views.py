...
def wear_list(request, song_cat=None, wear_category_id=None):
    ...
    categories = WearCategory.objects.filter(
        category_owner_id=request.user.id, activate=True)

    qs = Wear.objects.filter(user_id=request.user.id, activate=True)
    last_added_wear = Wear.objects.filter(
        user_id=request.user.id, activate=True)[:1]
    deactivated_wearss = Wear.objects.filter(
        user_id=request.user.id, activate=False).count()

    sum_all_n_wears = Wear.objects.filter(user_id=request.user.id, activate=True).aggregate(
        a=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))  # To sum all wears..
    ...
    if category != '' and category != None and category != 'Choose...':
        ...
        # i grab the value of the category selected above and push it in for category table using the id and from here, i can access all the atribute of the wearcategory table
        selected_category = get_object_or_404(WearCategory, id=category)

        # Suming amount based on the category selected
        selected_category = get_object_or_404(
            WearCategory, id=category, category_owner_id=request.user.id,)
        s_with_cat = qs.filter(category_id=category).aggregate(ba=Coalesce(Sum('amount'), V(
            0), output_field=DecimalField()))  # This will sum based on the category selected
    ...
    if start_date != '' and start_date != None:
        ...
        s_with_cat = qs.filter(date_created__gte=start_date).aggregate(baa=Coalesce(Sum('amount'), V(
            0), output_field=DecimalField()))  # This will sum based on the category selected
    ...
    if end_date != '' and end_date != None:
        ...
        s_with_cat = qs.filter(date_created__lt=end_date).aggregate(
            con=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('con')

    if search != '' and search != None:
        qs = qs.filter(Q(name__icontains=search) | Q(
            amount__icontains=search) | Q(bought_from__icontains=search))
        count_with_data_selected = qs.filter(Q(name__icontains=search) | Q(
            amount__icontains=search) | Q(bought_from__icontains=search))

        # summing amount based on the word search on
        selected_category = qs.filter(Q(name__icontains=search) | Q(
            amount__icontains=search) | Q(bought_from__icontains=search))
        s_with_cat = qs.filter(Q(name__icontains=search) | Q(amount__icontains=search) | Q(
            bought_from__icontains=search)).aggregate(ho=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    if expenses == 'on':
        ...
        s_with_cat = qs.filter(expenses=True).aggregate(
            ton=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        ...
        s_with_cat = qs.filter(expenses=False).aggregate(
            tah=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
...
def deactivate_wears(request, song_cat=None, wear_category_id=None):
    ...
    all_wear_categories = WearCategory.objects.filter(
        category_owner_id=request.user.id, activate=True)
    qs = Wear.objects.filter(user_id=request.user.id, activate=False)

    sum_all_d_wears = Wear.objects.filter(user_id=request.user.id, activate=False).aggregate(
        a=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))  # To sum all wears..
    ...
    if request.method == 'POST':
        ...
        if category != '':
            ...
            # i grab the value of the category selected above and push it in for category table using the id and from here, i can access all the atribute of the wearcategory table
            selected_category = get_object_or_404(WearCategory, id=category)
            s_with_cat = queryset.filter(category_id=category).aggregate(
                hu=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
            s_with_cat = s_with_cat.get('hu')

    wear_category = None
    # I am declaring a variable here otherwise, this variable will not be recognize in the function
    sum_wears_by_cat = None

    categories = WearCategory.objects.filter(
        category_owner_id=request.user.id, activate=False)
    wears = Wear.objects.filter(user_id=request.user.id, activate=False)

    all_wears = Wear.objects.filter(
        user_id=request.user.id, activate=False).count()
    all_wears_cat = WearCategory.objects.filter(
        category_owner_id=request.user.id, activate=False)

    sum_all_wears = Wear.objects.filter(user_id=request.user.id, activate=False).aggregate(
        oga=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))  # This is to sum all the wears
    # This is to get the dict key and used this key to display everything
    sum_all_wears = sum_all_wears.get('oga')

    deactivated_wearss = Wear.objects.filter(
        user_id=request.user.id, activate=False).count()
    deactivated_wears = Wear.objects.filter(
        user_id=request.user.id, activate=False)
    slice_deactivated_wears = Wear.objects.filter(
        user_id=request.user.id, activate=False)[:1]

    if wear_category_id:
        wear_category = get_object_or_404(
            WearCategory, id=wear_category_id, category_owner_id=request.user.id)
        wears = wears.filter(category=wear_category)
        sum_wears_by_cat = wears.filter(category=wear_category).aggregate(chai=Coalesce(Sum(
            'amount'), V(0), output_field=DecimalField()))  # sum wears based on the category filtered
        # get the dict key so it's can be display on the template
        sum_wears_by_cat = sum_wears_by_cat.get('chai')
...
def add_wear(request):
    if request.method == 'POST':
        wear_register_form = WearRegisterForm(
        ...
        if wear_register_form.is_valid():
            ...
            new_wear_reg.save()
            messages.success(
                request, f'{ new_wear_reg.name } wear was added successfully'.title())
...
def update_wear(request, wear_id):
    ...
    if request.method == 'POST':
        form = WearEditForm(request.user, request.POST,
                            request.FILES, instance=queryset)
        if form.is_valid():
            updated_wear = form.save(commit=False)
            updated_wear.user_id = request.user.id
            updated_wear.save()
            messages.success(
                request, f'{ updated_wear.name } was updated successfully by { request.user }'.title())
...
def delete_wear(request, id):
    ...
    if request.method == 'POST':
        get_id.delete()
        messages.success(
            request, f'{ get_id.name } wear was deleted successfully'.title())
        return redirect('homapp:wears_list')
    context = {
        'get_id': get_id,
    }
...
def add_category(request):
    cati = WearCategory.objects.all().filter(
        category_owner_id=request.user.id, activate=True).count()
    all_wears = Wear.objects.filter(
        user_id=request.user.id, activate=True).count()
    if request.method == 'POST':
        ...
        if cat_add_form.is_valid():
            ...
            new_cat.save()
            messages.success(
                request, f'{ new_cat.category_name } was added successfully'.title())
...
def category_list(request):
    ...
    all_wears = Wear.objects.filter(
        user_id=request.user.id, activate=True).count()
    last_add_category = WearCategory.objects.filter(
        category_owner_id=request.user.id, activate=True)[:1]
    deactivated_category = WearCategory.objects.filter(
        category_owner_id=request.user.id, activate=False).count()
    context = {
        'categories': categories,
        'cati': cati,
        'all_wears': all_wears,
...
def deactivated_category(request):
    all_deactivated_categories = WearCategory.objects.filter(
        category_owner_id=request.user.id, activate=False).count()
    all_activated_categories = WearCategory.objects.filter(
        category_owner_id=request.user.id, activate=True).count()
    all_deact_categories = WearCategory.objects.filter(
        category_owner_id=request.user.id, activate=False)
    last_added_category = WearCategory.objects.filter(
        category_owner_id=request.user.id, activate=True)[:1]
    total_wears = Wear.objects.filter(
        user_id=request.user.id, activate=True).count()
...
def edit_category(request, pk):
    ...
    queryset = get_object_or_404(
        WearCategory, id=pk, category_owner_id=request.user.id)
    ...
    if request.method == 'POST':
        ...
        if cat_edit_form.is_valid():
            ...
            new_category.save()
            messages.success(
                request, f'{ new_category.category_name } was successfully updated'.title())
            return redirect('homapp:category_list')
    context = {
...
def delete_category(request, pk):
    ...
    if request.method == 'POST':
        queryset.delete()
        messages.success(
            request, f'{queryset.category_name} was deleted successfully'.title())
        return redirect('homapp:category_list')
    context = {
        'queryset': queryset,
    }
...
def finance_list(request):
    ...
    all_fin_categories = FinanceCategory.objects.filter(
        activate=True, category_owner_id=request.user.id)
    # To show the last added objects and slice the result to be one showing...
    last_added_finance = Finance.objects.filter(
        activate=True, user_id=request.user.id)[:1]
    deactivate_finances_count = Finance.objects.filter(
        activate=False, user_id=request.user.id).count()
    ...
    if category != '' and category != None and category != 'Choose...':
        ...
        # Suming amount based on the category selected
        selected_category = get_object_or_404(FinanceCategory, id=category)
        s_with_cat = finances.filter(category_id=category).aggregate(ba=Coalesce(Sum('amount'), V(
            0), output_field=DecimalField()))  # This will sum based on the category selected
        s_with_cat = s_with_cat.get('ba')

    if start_date != '' and start_date != None:
        finances = finances.filter(date_created__gte=start_date)
        count_finance_data_selected = finances.filter(
            date_created__gte=start_date)

        # Suming amount based on only the start date selected
        selected_category = finances.filter(date_created__gte=start_date)
        s_with_cat = finances.filter(date_created__gte=start_date).aggregate(baa=Coalesce(Sum(
            'amount'), V(0), output_field=DecimalField()))  # This will sum based on the category selected
        s_with_cat = s_with_cat.get('baa')
    ...
    if end_date != '' and end_date != None:
        ...
        s_with_cat = finances.filter(date_created__lt=end_date).aggregate(
            con=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('con')
    ...
    if search != '' and search != None:
        ...
        s_with_cat = finances.filter(title__icontains=search).aggregate(
            hjo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    if expenses == 'on':
        ...
        s_with_cat = finances.filter(expenses=True).aggregate(
            ton=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('ton')
        ...
        s_with_cat = finances.filter(expenses=False).aggregate(
            tah=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('tah')
    ...
    if request.method == 'POST':
        ...
        if category != '':
            finances = finances.filter(category_id=category)
            # This will count the category based on what is passed in..
            count_with_category = finances.filter(category_id=category)
            # i grab the value of the category selected above and push it in for category table using the id and from here, i can access all the atribute of the wearcategory table
            selected_category = get_object_or_404(FinanceCategory, id=category)

            s_with_cat = finances.filter(category_id=category).aggregate(hu=Coalesce(Sum('amount'), V(
                0), output_field=DecimalField()))  # This will sum based on the category selected
            s_with_cat = s_with_cat.get('hu')

    sum_all_finances = Finance.objects.filter(user_id=request.user.id, activate=True).aggregate(
        oga=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))  # This is to sum all the Finance
    # This is to get the dict key and used this key to display everything
    sum_all_finances = sum_all_finances.get('oga')
...
def add_finance(request):
    if request.method == 'POST':
        finance_add_form = FinanceAddForm(
            request.user, request.POST, request.FILES)
        if finance_add_form.is_valid():
            ...
            new_finance.save()
            messages.success(
                request, f'{new_finance.title} was added successfully'.title())
...
def edit_finance(request, pk):
    ...
    if request.method == 'POST':
        finance_edit_form = EditFinanceForm(
            request.user, request.POST, request.FILES, instance=queryset)
        if finance_edit_form.is_valid():
            new_form = finance_edit_form.save(commit=False)
            new_form.user_id = request.user.id
            new_form.save()
            messages.success(
                request, f'{new_form.title} was updated successfully'.title())
            return redirect('homapp:all_transactions')
    context = {
        'new_form': queryset,
...
def delete_finance(request, pk):
    ...
    if request.method == 'POST':
        queryset.delete()
        messages.success(
            request, f'{queryset.title} was deleted successfully'.title())
        return redirect('homapp:all_transactions')
    context = {
        'queryset': queryset,
    }
...
def finance_cat_list(request):
    all_fin_categories = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=True)
    finances = Finance.objects.filter(user_id=request.user.id, activate=True)
    all_fin_cat = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=True).count()
    last_added_fin_category = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=True)[:1]
    all_deactivated_category = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=False).count()
...
def add_fin_category(request):
    if request.method == 'POST':
        ...
        if add_fin_category_form.is_valid():
            ...
            new_fin_cat.save()
            messages.success(
                request, f'{new_fin_cat.category_name} was added successfully'.title())
...
def edit_fin_category(request, pk):
    ...
    if request.method == 'POST':
        ...
        if edit_fin_form.is_valid():
            ...
            new_edit.save()
            messages.success(
                request, f'{new_edit.category_name} was updated successfully'.title())
            return redirect('homapp:finance_cat_list')
    context = {
        'new_edit': queryset,
...
def delete_fin_category(request, pk):
    queryset = get_object_or_404(
        FinanceCategory, id=pk, category_owner_id=request.user.id)
    if request.method == 'POST':
        queryset.delete()
        messages.success(
            request, f'{queryset.category_name} was deleted successfully'.title())
...
def deactivated_finances(request):
    ...
    all_deactivated_finances = Finance.objects.filter(
        user_id=request.user.id, activate=False)

    # Am summing all deactivated finance amount
    sum_all_dect_finances = Finance.objects.filter(user_id=request.user.id, activate=False).aggregate(
        zy=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))  # This is to sum all the Finance
    # This is to get the dict key and used this key to display everything
    sum_all_dect_finances = sum_all_dect_finances.get('zy')
    ...
    # To show the last added objects and slice the result to be one showing...
    last_deactivated_finance = Finance.objects.filter(
        user_id=request.user.id, activate=False)[:1]

    deactivate_finances_count = Finance.objects.filter(
        user_id=request.user.id, activate=False).count()
    # all total number of finance
    total_n_finance = Finance.objects.filter(
        user_id=request.user.id, activate=True).count()
    ...
    if category != '' and category != None and category != 'Choose...':
        all_deactivated_finances = all_deactivated_finances.filter(
            user_id=request.user.id, category_id=category)
        count_finance_data_selected = all_deactivated_finances.filter(
            user_id=request.user.id, category_id=category)
        # i grab the value of the category selected above and push it in for category table using the id and from here, i can access all the atribute of the wearcategory table
        selected_category = get_object_or_404(FinanceCategory, id=category)
        ...
        # Suming amount based on the category selected
        selected_category = get_object_or_404(FinanceCategory, id=category)
        s_with_cat = all_deactivated_finances.filter(user_id=request.user.id, category_id=category).aggregate(
            ba=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))  # This will sum based on the category selected
        s_with_cat = s_with_cat.get('ba')

    if start_date != '' and start_date != None:
        all_deactivated_finances = all_deactivated_finances.filter(
            user_id=request.user.id, date_created__gte=start_date)
        count_finance_data_selected = all_deactivated_finances.filter(
            user_id=request.user.id, date_created__gte=start_date)

        # Suming amount based on only the start date selected
        selected_category = all_deactivated_finances.filter(
            user_id=request.user.id, date_created__gte=start_date)
        s_with_cat = all_deactivated_finances.filter(user_id=request.user.id, date_created__gte=start_date).aggregate(
            baa=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))  # This will sum based on the category selected
        s_with_cat = s_with_cat.get('baa')

    if end_date != '' and end_date != None:
        all_deactivated_finances = all_deactivated_finances.filter(
            user_id=request.user.id, date_created__lt=end_date)
        count_finance_data_selected = all_deactivated_finances.filter(
            user_id=request.user.id, date_created__lt=end_date)
        ...
        selected_category = all_deactivated_finances.filter(
            user_id=request.user.id, date_created__lt=end_date)
        s_with_cat = all_deactivated_finances.filter(user_id=request.user.id, date_created__lt=end_date).aggregate(
            con=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('con')
    ...
    if search != '' and search != None:
        ...
        selected_category = all_deactivated_finances.filter(
            user_id=request.user.id, title__icontains=search)
        s_with_cat = all_deactivated_finances.filter(user_id=request.user.id, title__icontains=search).aggregate(
            ho=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('ho')

    if expenses == 'on':
        all_deactivated_finances = all_deactivated_finances.filter(
            user_id=request.user.id, expenses=True)
        count_finance_data_selected = all_deactivated_finances.filter(
            user_id=request.user.id, expenses=True)
        ...
        selected_category = all_deactivated_finances.filter(
            user_id=request.user.id, expenses=True)
        s_with_cat = all_deactivated_finances.filter(user_id=request.user.id, expenses=True).aggregate(
            ton=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('ton')

    elif not_expenses == 'on':
        all_deactivated_finances = all_deactivated_finances.filter(
            user_id=request.user.id, expenses=False)
        count_finance_data_selected = all_deactivated_finances.filter(
            user_id=request.user.id, expenses=False)
        ...
        selected_category = all_deactivated_finances.filter(
            user_id=request.user.id, expenses=False)
        s_with_cat = all_deactivated_finances.filter(user_id=request.user.id, expenses=False).aggregate(
            tah=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('tah')
...
def edit_finance_deactivated(request, pk):
    queryset = get_object_or_404(Finance, id=pk)
    finance_deactivated_edit_form = EditFinanceDectForm(
        request.user, instance=queryset)
    if request.method == 'POST':
        finance_deactivated_edit_form = EditFinanceDectForm(
            request.user, request.POST, instance=queryset)
...
def delete_deactivated_finance(request, pk):
    ...
    if request.method == 'POST':
        qs.delete()
        messages.success(
            request, f'{qs.title} was deleted successfully'.title())
        return redirect('homapp:deactivated_finances')
    context = {
        'qs': qs,
    }
...
def deactivated_finance_category(request):
    all_deactivated_finance_categories = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=False).count()
    all_activated_finance_categories = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=True).count()
    all_deact_finance_categories = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=False)
    last_added_finance_category = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=False)[:1]
    total_finance = Finance.objects.filter(
        user_id=request.user.id, activate=True).count()
...
def finance_deactivated_edit_category(request, pk):
    ...
    total_finance_categories = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=True).count()
    ...
    last_deactivated_category = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=False)[:1]

    finance_deactivated_cat = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=False).count()
    queryset = get_object_or_404(
        FinanceCategory, category_owner_id=request.user.id, activate=False, id=pk)
    finance_deactivated_category_form = FinanceDeactivatedCatForm(
        instance=queryset)
    if request.method == 'POST':
        finance_deactivated_category_form = FinanceDeactivatedCatForm(
        ...
        if finance_deactivated_category_form.is_valid():
            ...
            new_fin_dect.save()
            messages.success(
                request, f'{ new_fin_dect.category_name } was successfully updated'.title())
...
def all_transactions(request):
    ...
    finances = Finance.objects.filter(user_id=request.user.id, activate=True)
    c_expenses = Finance.objects.filter(
        user_id=request.user.id, expenses=True).count()
    c_income = Finance.objects.filter(
        user_id=request.user.id, expenses=False).count()
    all_fin_categories = FinanceCategory.objects.filter(
        category_owner_id=request.user.id, activate=True)

    sum_expenses = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=True).aggregate(
        x=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    sum_expenses = sum_expenses.get('x')

    sum_income = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=False).aggregate(
        y=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    sum_income = sum_income.get('y')

    sum_all_finances = Finance.objects.filter(user_id=request.user.id, activate=True).aggregate(
        z=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))  # This is to sum all the Finance
    # This is to get the dict key and used this key to display everything
    sum_all_finances = sum_all_finances.get('z')
    ...
    if category != '' and category != None and category != 'Choose...':
        ...
        # Suming amount based on the category selected
        selected_category = get_object_or_404(FinanceCategory, id=category)
        s_with_cat = finances.filter(user_id=request.user.id, category_id=category).aggregate(ba=Coalesce(
            Sum('amount'), V(0), output_field=DecimalField()))  # This will sum based on the category selected
        s_with_cat = s_with_cat.get('ba')
    ...
    if start_date != '' and start_date != None:
        ...
        # Suming amount based on only the start date selected
        selected_category = finances.filter(
            user_id=request.user.id, date_created__gte=start_date)
        s_with_cat = finances.filter(user_id=request.user.id, date_created__gte=start_date).aggregate(baa=Coalesce(
            Sum('amount'), V(0), output_field=DecimalField()))  # This will sum based on the category selected
        s_with_cat = s_with_cat.get('baa')
    ...
    if end_date != '' and end_date != None:
        ...
        selected_category = finances.filter(
            user_id=request.user.id, date_created__lt=end_date)
        s_with_cat = finances.filter(user_id=request.user.id, date_created__lt=end_date).aggregate(
            con=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('con')

    if search != '' and search != None:
        finances = finances.filter(Q(title__icontains=search) | Q(
            amount__icontains=search), user_id=request.user.id)
        count_finance_data_selected = finances.filter(Q(title__icontains=search) | Q(
            amount__icontains=search), user_id=request.user.id)

        # summing amount based on the word search on
        selected_category = finances.filter(Q(title__icontains=search) | Q(
            amount__icontains=search), user_id=request.user.id)
        s_with_cat = finances.filter(user_id=request.user.id, title__icontains=search).aggregate(
            ho=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('ho')
    ...
    if expenses == 'on':
        ...
        selected_category = finances.filter(
            user_id=request.user.id, expenses=True)
        s_with_cat = finances.filter(user_id=request.user.id, expenses=True).aggregate(
            ton=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('ton')
        ...
        selected_category = finances.filter(
            user_id=request.user.id, expenses=False)
        s_with_cat = finances.filter(user_id=request.user.id, expenses=False).aggregate(
            tah=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
        s_with_cat = s_with_cat.get('tah')
    ...
    wear_total_amount = Wear.objects.aggregate(g=Coalesce(
        Sum('amount'), V(0), output_field=DecimalField()))
    wear_total_amount = wear_total_amount.get('g')

    # I sum the total amount in finance table here......
    finance_total_amount = Finance.objects.aggregate(
        w=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    wear_income = Wear.objects.filter(expenses=False).aggregate(
        xo=Coalesce(Sum('amount'), V(0)))
    wear_income = wear_income.get('xo')

    # I sum the total income in finance table
    finance_income = Finance.objects.filter(
        expenses=False).aggregate(yo=Coalesce(Sum('amount'), V(0)))
    ...
    wear_expenses = Wear.objects.filter(expenses=True).aggregate(
        xo=Coalesce(Sum('amount'), V(0)))
    wear_expenses = wear_expenses.get('xo')

    # I sum the total expenses in finance table

    finance_expenses = Finance.objects.filter(
        expenses=True).aggregate(yo=Coalesce(Sum('amount'), V(0)))
...
def today_summary(request):
    ...
    count_today_wear_income = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__date=today_date).count()
    count_today_finance_income = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__date=today_date).count()
    all_today_income_count = count_today_wear_income + count_today_finance_income

    count_today_wear_expenses = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__date=today_date).count()
    count_today_finance_expenses = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__date=today_date).count()
    all_today_expenses_count = count_today_wear_expenses + count_today_finance_expenses

    counting_today_wear_data = Wear.objects.filter(
        user_id=request.user.id, activate=True, date_created__date=today_date).count()
    counting_today_finance_data = Finance.objects.filter(
        user_id=request.user.id, activate=True, date_created__date=today_date).count()
    all_data_for_today = counting_today_wear_data + counting_today_finance_data

    # getting wear total income for today alone
    wear_total_income_amount_for_today = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__date=today_date).aggregate(
        xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    wear_total_income_amount_for_today = wear_total_income_amount_for_today.get(
        'xo')

    # getting finance total income for today alone
    finance_total_income_amount_for_today = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__date=today_date).aggregate(
        yo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    finance_total_income_amount_for_today = finance_total_income_amount_for_today.get(
        'yo')

    today_total_income = wear_total_income_amount_for_today + \
        finance_total_income_amount_for_today

    # getting wear total expenses for today alone
    wear_total_expenses_amount_for_today = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__date=today_date).aggregate(
        xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    wear_total_expenses_amount_for_today = wear_total_expenses_amount_for_today.get(
        'xo')

    # getting finance total expenses for today alone
    finance_total_expenses_amount_for_today = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__date=today_date).aggregate(yo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    finance_total_expenses_amount_for_today = finance_total_expenses_amount_for_today.get(
        'yo')

    today_total_expenses = wear_total_expenses_amount_for_today + \
        finance_total_expenses_amount_for_today

    all_amount_for_today = today_total_income + today_total_expenses

    # Today wear total amount
    today_wear_total_amount = Wear.objects.filter(user_id=request.user.id, activate=True, date_created__date=today_date).aggregate(
        ty=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    today_wear_total_amount = today_wear_total_amount.get('ty')

    # Today Finance total amount
    today_finance_total_amount = Finance.objects.filter(user_id=request.user.id, activate=True, date_created__date=today_date).aggregate(
        tp=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
...
def last_3_days(request):
    ...
    last_3_days_wear_income_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__range=(last_3_days, today)).count()

    # counting last 3 days wear income alone
    last_3_days_finance_income_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__range=(last_3_days, today)).count()

    # summing up last 3 days income from both wears and finance together
    income_for_last_3_days = last_3_days_wear_income_count + \
        last_3_days_finance_income_count

    # counting last 3 days wear expenses alone
    last_3_days_wear_expenses_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__range=(last_3_days, today)).count()

    # counting last 3 days finance expenses alone
    last_3_days_finance_expenses_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__range=(last_3_days, today)).count()

    # summing up last 3 days expenses from both wears and finance together
    expenses_for_last_3_days = last_3_days_wear_expenses_count + \
        last_3_days_finance_expenses_count

    last_3_days_wear_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, date_created__range=(last_3_days, today)).count()
    last_3_days_finance_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, date_created__range=(last_3_days, today)).count()
    ...
    last_3_days_wear_income = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__range=(
        last_3_days, today)).aggregate(xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    last_3_days_wear_income = last_3_days_wear_income.get('xo')

    # Last 3 days Finance income
    last_3_days_finance_income = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__range=(
        last_3_days, today)).aggregate(yo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    last_3_days_wear_expenses = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__range=(
        last_3_days, today)).aggregate(xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    last_3_days_wear_expenses = last_3_days_wear_expenses.get('xo')

    # last 3 days finance expenses
    last_3_days_finance_expenses = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__range=(
        last_3_days, today)).aggregate(uo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    last_3_days_wear_total_amount = Wear.objects.filter(user_id=request.user.id, activate=True, date_created__range=(
        last_3_days, today)).aggregate(ro=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    last_3_days_wear_total_amount = last_3_days_wear_total_amount.get('ro')

    # last 3 days finance total Amount
    last_3_days_finance_total_amount = Finance.objects.filter(user_id=request.user.id, activate=True, date_created__range=(
        last_3_days, today)).aggregate(ra=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    last_3_days_finance_total_amount = last_3_days_finance_total_amount.get(
        'ra')
    ...
    context = {
        ...
        # counting all wear for the last 3 days
        'last_3_days_wear_count': last_3_days_wear_count,
        # counting all finance for the last 3 days
        'last_3_days_finance_count': last_3_days_finance_count,
        # summing all of the to get the total data for the last 3 days
        'last_3_days_data_count': last_3_days_data_count,
...
def last_7_days(request):
    ...
    last_7_days_wear_income_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__range=(last_7_days, today)).count()

    # counting last 7 days wear income alone
    last_7_days_finance_income_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__range=(last_7_days, today)).count()

    # summing up last 7 days income from both wears and finance together
    income_for_last_7_days = last_7_days_wear_income_count + \
        last_7_days_finance_income_count

    # counting last 7 days wear expenses alone
    last_7_days_wear_expenses_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__range=(last_7_days, today)).count()

    # counting last 7 days finance expenses alone
    last_7_days_finance_expenses_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__range=(last_7_days, today)).count()

    # summing up last 7 days expenses from both wears and finance together
    expenses_for_last_7_days = last_7_days_wear_expenses_count + \
        last_7_days_finance_expenses_count

    last_7_days_wear_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, date_created__range=(last_7_days, today)).count()
    last_7_days_finance_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, date_created__range=(last_7_days, today)).count()
    ...
    last_7_days_wear_income = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__range=(
        last_7_days, today)).aggregate(xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    last_7_days_wear_income = last_7_days_wear_income.get('xo')

    # Last 7 days Finance income
    last_7_days_finance_income = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__range=(
        last_7_days, today)).aggregate(yo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    last_7_days_wear_expenses = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__range=(
        last_7_days, today)).aggregate(xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    last_7_days_wear_expenses = last_7_days_wear_expenses.get('xo')

    # last 7 days finance expenses
    last_7_days_finance_expenses = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__range=(
        last_7_days, today)).aggregate(uo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    last_7_days_wear_total_amount = Wear.objects.filter(user_id=request.user.id, activate=True, date_created__range=(
        last_7_days, today)).aggregate(ho=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    last_7_days_wear_total_amount = last_7_days_wear_total_amount.get('ho')

    # Last 7 days finance total amount
    last_7_days_finance_total_amount = Finance.objects.filter(user_id=request.user.id, activate=True, date_created__range=(
        last_7_days, today)).aggregate(zo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    last_7_days_finance_total_amount = last_7_days_finance_total_amount.get(
        'zo')
    ...
    context = {
        ...
        # counting all wear for the last 7 days
        'last_7_days_wear_count': last_7_days_wear_count,
        # counting all finance for the last 7 days
        'last_7_days_finance_count': last_7_days_finance_count,
        # summing all of the to get the total data for the last 7 days
        'last_7_days_data_count': last_7_days_data_count,
...
def current_week(request):
    ...
    current_week_wear_income_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__week=current_week).count()

    # counting current week wear income alone
    current_week_finance_income_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__week=current_week).count()

    # summing up current week income from both wears and finance together
    income_for_current_week = current_week_wear_income_count + \
        current_week_finance_income_count

    # counting current week wear expenses alone
    current_week_wear_expenses_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__week=current_week).count()

    # counting current week finance expenses alone
    current_week_finance_expenses_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__week=current_week).count()

    # summing up current week expenses from both wears and finance together
    expenses_for_current_week = current_week_wear_expenses_count + \
        current_week_finance_expenses_count

    current_week_wear_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, date_created__week=current_week).count()

    current_week_finance_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, date_created__week=current_week).count()
    ...
    current_week_wear_income = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__week=current_week).aggregate(
        xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_week_wear_income = current_week_wear_income.get('xo')

    # current week Finance income
    current_week_finance_income = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__week=current_week).aggregate(
        yo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    current_week_wear_expenses = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__week=current_week).aggregate(
        xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_week_wear_expenses = current_week_wear_expenses.get('xo')

    # current week finance expenses
    current_week_finance_expenses = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__week=current_week).aggregate(
        uo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_week_finance_expenses = current_week_finance_expenses.get('uo')

    # sum all current week expenses together
    sum_current_week_expenses = current_week_wear_expenses + \
        current_week_finance_expenses
    ...
    current_week_wear_total_amount = Wear.objects.filter(user_id=request.user.id, activate=True, date_created__week=current_week).aggregate(
        ho=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_week_wear_total_amount = current_week_wear_total_amount.get('ho')

    # This current week finance total amount
    current_week_finance_total_amount = Finance.objects.filter(user_id=request.user.id, activate=True, date_created__week=current_week).aggregate(
        zo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_week_finance_total_amount = current_week_finance_total_amount.get(
        'zo')
    ...
    context = {
        ...
        # counting all wear for the last 7 days
        'current_week_wear_count': current_week_wear_count,
        # counting all finance for the last 7 days
        'current_week_finance_count': current_week_finance_count,
        # summing all of the to get the total data for the last 7 days
        'current_week_data_count': current_week_data_count,
...
def current_month(request):
    ...
    # counting last 7 days wear income alone
    current_month_wear_income_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__month=current_month).count()

    # counting last 7 days wear income alone
    current_month_finance_income_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__month=current_month).count()

    # summing up last 7 days income from both wears and finance together
    income_for_current_month = current_month_wear_income_count + \
        current_month_finance_income_count

    # counting last 7 days wear expenses alone
    current_month_wear_expenses_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__month=current_month).count()

    # counting last 7 days finance expenses alone
    current_month_finance_expenses_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__month=current_month).count()

    # summing up last 7 days expenses from both wears and finance together
    expenses_for_current_month = current_month_wear_expenses_count + \
        current_month_finance_expenses_count

    current_month_wear_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, date_created__month=current_month).count()

    current_month_finance_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, date_created__month=current_month).count()
    ...
    # Last 7 days wear income
    current_month_wear_income = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__month=current_month).aggregate(
        xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_month_wear_income = current_month_wear_income.get('xo')

    # Last 7 days Finance income
    current_month_finance_income = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__month=current_month).aggregate(
        yo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    # last 7 days wear expenses
    current_month_wear_expenses = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__month=current_month).aggregate(
        xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_month_wear_expenses = current_month_wear_expenses.get('xo')

    # last 7 days finance expenses
    current_month_finance_expenses = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__month=current_month).aggregate(
        uo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_month_finance_expenses = current_month_finance_expenses.get('uo')

    # sum all last_7_days expenses together
    sum_current_month_expenses = current_month_wear_expenses + \
        current_month_finance_expenses

    # summing amount for last 7 days
    total_amount_for_current_month = sum_current_month_income + sum_current_month_expenses

    # This month wear total amount
    current_month_wear_total_amount = Wear.objects.filter(user_id=request.user.id, activate=True, date_created__month=current_month).aggregate(
        ho=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_month_wear_total_amount = current_month_wear_total_amount.get('ho')

    # This month finance total amount
    current_month_finance_total_amount = Finance.objects.filter(user_id=request.user.id, activate=True, date_created__month=current_month).aggregate(
        zo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_month_finance_total_amount = current_month_finance_total_amount.get(
        'zo')
    ...
    context = {
        ...
        # last 7 days data count
        # counting all wear for the last 7 days
        'current_month_wear_count': current_month_wear_count,
        # counting all finance for the last 7 days
        'current_month_finance_count': current_month_finance_count,
        # summing all of the to get the total data for the last 7 days
        'current_month_data_count': current_month_data_count,
...
def current_year(request):
    ...
    current_year_wear_income_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__year=current_year).count()

    # counting this year wear income alone
    current_year_finance_income_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=False, date_created__year=current_year).count()

    # summing up this year income from both wears and finance together
    income_for_current_year = current_year_wear_income_count + \
        current_year_finance_income_count

    # counting this year wear expenses alone
    current_year_wear_expenses_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__year=current_year).count()

    # counting this year finance expenses alone
    current_year_finance_expenses_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, expenses=True, date_created__year=current_year).count()

    # summing up this year expenses from both wears and finance together
    expenses_for_current_year = current_year_wear_expenses_count + \
        current_year_finance_expenses_count

    current_year_wear_count = Wear.objects.filter(
        user_id=request.user.id, activate=True, date_created__year=current_year).count()

    current_year_finance_count = Finance.objects.filter(
        user_id=request.user.id, activate=True, date_created__year=current_year).count()
    ...
    current_year_wear_income = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__year=current_year).aggregate(
        xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_year_wear_income = current_year_wear_income.get('xo')

    # this year Finance income
    current_year_finance_income = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=False, date_created__year=current_year).aggregate(
        yo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    current_year_wear_expenses = Wear.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__year=current_year).aggregate(
        xo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_year_wear_expenses = current_year_wear_expenses.get('xo')

    # this year finance expenses
    current_year_finance_expenses = Finance.objects.filter(user_id=request.user.id, activate=True, expenses=True, date_created__year=current_year).aggregate(
        uo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_year_finance_expenses = current_year_finance_expenses.get('uo')

    # sum all this year expenses together
    sum_current_year_expenses = current_year_wear_expenses + \
        current_year_finance_expenses
    ...
    current_year_wear_total_amount = Wear.objects.filter(user_id=request.user.id, activate=True, date_created__year=current_year).aggregate(
        ho=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_year_wear_total_amount = current_year_wear_total_amount.get('ho')

    # This year finance total amount
    current_year_finance_total_amount = Finance.objects.filter(user_id=request.user.id, activate=True, date_created__year=current_year).aggregate(
        zo=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    current_year_finance_total_amount = current_year_finance_total_amount.get(
        'zo')
    ...
    context = {
        ...
        # counting all wear for the last 7 days
        'current_year_wear_count': current_year_wear_count,
        # counting all finance for the last 7 days
        'current_year_finance_count': current_year_finance_count,
        # summing all of the to get the total data for the last 7 days
        'current_year_data_count': current_year_data_count,
...
def list_task(request):
    ...
    all_task = Task.objects.filter(
        user_id=request.user.id, activate=True).order_by('complete', 'due')
    ...
    uncompleted_tasks = Task.objects.filter(
        user_id=request.user.id, activate=True, complete=False).count()

    # counting all completed tasks
    completed_tasks = Task.objects.filter(
        user_id=request.user.id, activate=True, complete=True).count()

    # All deactivated tasks
    deactivated_tasks = Task.objects.filter(
        user_id=request.user.id, activate=False).count()
...
def update_todo_task(request, pk):
    qs = get_object_or_404(Task, activate=True,  id=pk,
                           user_id=request.user.id)
    todo_update_form = UpdateTodoForm(request.user, instance=qs)
    # Processing the data inside the form
    if request.method == 'POST':
        todo_update_form = UpdateTodoForm(
            request.user, request.POST, instance=qs)
...
def delete_task(request, pk):
    ...
    if request.method == 'POST':
        qs.delete()
        messages.success(
            request, f'{qs.name} was deleted successfully'.title())
        return redirect('homapp:list_task')
    context = {
        'qs': qs,
    }
...
def list_task_category(request):
    ...
    all_task_category = TaskCategory.objects.filter(
        user_id=request.user.id, activate=True).order_by('-date_created')
    deactivated_task_categories = TaskCategory.objects.filter(
        user_id=request.user.id, activate=False).order_by('-date_created').count()
    ...
    if start_date != '' and start_date != None:
        all_task_category = all_task_category.filter(
            activate=True, user_id=request.user.id, date_created__gte=start_date)
        count_task_filtered = all_task_category.filter(
            activate=True, user_id=request.user.id, date_created__gte=start_date)

    if end_date != '' and end_date != None:
        all_task_category = all_task_category.filter(
            activate=True, user_id=request.user.id, date_created__lt=end_date)
        count_task_filtered = all_task_category.filter(
            activate=True, user_id=request.user.id, date_created__lt=end_date)

    if search != '' and search != None:
        all_task_category = all_task_category.filter(
            activate=True, user_id=request.user.id, name__icontains=search)
        count_task_filtered = all_task_category.filter(
            activate=True, user_id=request.user.id, name__icontains=search)

    # last added task category
    last_added_task_category = TaskCategory.objects.filter(
        user_id=request.user.id, activate=True).order_by('-date_created')[:1]
...
def deactivated_task(request):
    ...
    all_deactivated_task = Task.objects.filter(
        user_id=request.user.id, activate=False).order_by('complete', 'due')
    ...
    # counting all tasks
    all_deactivated_task_count = Task.objects.filter(
        user_id=request.user.id, activate=False)

    # Counting all uncompleted Tasks
    uncompleted_deactivated_tasks = Task.objects.filter(
        user_id=request.user.id, activate=False, complete=False).count()

    # counting all completed tasks
    completed_deactivated_tasks = Task.objects.filter(
        user_id=request.user.id, activate=False, complete=True).count()

    # All deactivated tasks
    activated_tasks = Task.objects.filter(
        user_id=request.user.id, activate=True).count()
    ...
    if category != '' and category != None and category != 'Choose...':
        all_deactivated_task = all_deactivated_task.filter(
    ...
    if start_date != '' and start_date != None:
        all_deactivated_task = all_deactivated_task.filter(
            date_created__gte=start_date)
        count_task_filtered = all_deactivated_task.filter(
            date_created__gte=start_date)
...
def deactivated_task_category(request):
    ...
    all_deactivated_task_category = TaskCategory.objects.filter(
        user_id=request.user.id, activate=False).order_by('-date_created')
    activated_task_categories = TaskCategory.objects.filter(
        user_id=request.user.id, activate=True).order_by('-date_created').count()
    ...
    if start_date != '' and start_date != None:
        all_deactivated_task_category = all_deactivated_task_category.filter(
            activate=False, user_id=request.user.id, date_created__gte=start_date)
        count_task_filtered = all_deactivated_task_category.filter(
            activate=False, user_id=request.user.id, date_created__gte=start_date)

    if end_date != '' and end_date != None:
        all_deactivated_task_category = all_deactivated_task_category.filter(
            activate=False, user_id=request.user.id, date_created__lt=end_date)
        count_task_filtered = all_deactivated_task_category.filter(
            activate=False, user_id=request.user.id, date_created__lt=end_date)

    if search != '' and search != None:
        all_deactivated_task_category = all_deactivated_task_category.filter(
            activate=False, user_id=request.user.id, name__icontains=search)
        count_task_filtered = all_deactivated_task_category.filter(
            activate=False, user_id=request.user.id, name__icontains=search)
...
def update_task_deactivated_category(request, pk):
    qs = get_object_or_404(TaskCategory, activate=False,
                           id=pk, user_id=request.user.id)
    update_task_category_form = UpdateTaskCategoryForm(instance=qs)
    if request.method == 'POST':
        update_task_category_form = UpdateTaskCategoryForm(
...
def all_wears_for_admin(request):
    ...
    if category != '' and category != None and category != 'Choose...':
        ...
        count_with_data_selected = all_wears_for_admin.filter(
            category_id=category)
        # i grab the value of the category selected above and push it in for category table using the id and from here, i can access all the atribute of the wearcategory table
        selected_category = get_object_or_404(WearCategory, id=category)

    if start_date != '' and start_date != None:
        all_wears_for_admin = all_wears_for_admin.filter(
            date_created__gte=start_date)
        count_with_data_selected = all_wears_for_admin.filter(
            date_created__gte=start_date)
...
def admin_edit_wear(request, pk):
    ...
    if request.method == 'POST':
        ...
        if admin_edit_wear_form.is_valid():
            ...
            messages.success(
                request, f'wear for {get_user_username.user.username} was updated successfully')
...
def admin_delete_wear(request, pk):
    ...
    if request.method == 'POST':
        qs.delete()
        messages.success(
            request, f'wear was deleted successfully for {qs.user.username}')
        return redirect('homapp:all_wears_for_admin')
...
def all_wears_categories_for_admin(request):
    ...
    total_users_category_owner = WearCategory.objects.values_list(
        'category_owner', flat=True).distinct()
    total_wears_categories = WearCategory.objects.all()
    total_deactivated_wears_categories = WearCategory.objects.filter(
        activate=False)
    ...
    if start_date != '' and start_date != None:
        total_wears_categories = total_wears_categories.filter(
            date_created__gte=start_date)
        count_with_data_selected = total_wears_categories.filter(
            date_created__gte=start_date)
...
def admin_add_wear_category(request):
    ...
    if request.method == 'POST':
        ...
        if admin_add_wear_category_form.is_valid():
            ...
            get_user_username.save()
            messages.success(
                request, f'{get_user_username.category_name} wear category was added successfully for {get_user_username.category_owner.username}')
...
def admin_edit_wear_category(request, pk):
    ...
    if request.method == 'POST':
        admin_edit_wear_category_form = AdminEditWearCategoryForm(
            request.POST, instance=qs)
        if admin_edit_wear_category_form.is_valid():
            get_user_username = admin_edit_wear_category_form.save(
                commit=False)
            get_user_username.save()
            messages.success(
                request, f'{get_user_username.category_name} category was updated successfully for { get_user_username.category_owner.username }')
            return redirect('homapp:all_wears_categories_for_admin')
...
def admin_delete_wear_category(request, pk):
    ...
    if request.method == 'POST':
        qs.delete()
        messages.success(
            request, f'{qs.category_name} category was deleted successfully for {qs.category_owner.username }')
        return redirect('homapp:all_wears_categories_for_admin')
...
def all_finance_for_admin(request):
    ...
    total_amount_for_finance = Finance.objects.all().aggregate(
        p=Coalesce(Sum('amount'), V(0), output_field=DecimalField()))
    ...
    if category != '' and category != None and category != 'Choose...':
        all_finance_for_admin = all_finance_for_admin.filter(
            category_id=category)
        count_with_data_selected = all_finance_for_admin.filter(
            category_id=category)
        # i grab the value of the category selected above and push it in for category table using the id and from here, i can access all the atribute of the wearcategory table
        selected_category = get_object_or_404(WearCategory, id=category)
...
def admin_add_finance(request):
    ...
    if request.method == 'POST':
        ...
        if admin_add_finance_form.is_valid():
            ...
            messages.success(
                request, f'finance added for {finance_for_user.user.username}')
...
def admin_edit_finance(request, pk):
    ...
    if request.method == 'POST':
        admin_edit_finance_form = AdminEditFinanceForm(
            request.POST, instance=qs)
        if admin_edit_finance_form.is_valid():
            get_user = admin_edit_finance_form.save(commit=False)
            get_user.save()
            messages.success(
                request, f'finance for {get_user.user.username} was updated successfully')
            return redirect('homapp:all_finance_for_admin')
...
def admin_delete_finance(request, pk):
    ...
    if request.method == 'POST':
        qs.delete()
        messages.success(
            request, f'finance for {qs.user.username} was deleted successfully')
        return redirect('homapp:all_finance_for_admin')
...
def all_finance_categories_for_admin(request):
    ...
    remember that a single user can have many categories created by only him/her, so i needed a way to count only those 
    categories creators only """
    total_finance_category_owner = FinanceCategory.objects.values_list(
        'category_owner', flat=True).distinct()
    total_finance_categories = FinanceCategory.objects.all()
    total_deactivated_finance_categories = FinanceCategory.objects.filter(
        activate=False)
    ...
    if start_date != '' and start_date != None:
        total_finance_categories = total_finance_categories.filter(
            date_created__gte=start_date)
        count_with_data_selected = total_finance_categories.filter(
            date_created__gte=start_date)
...
def admin_add_finance_category(request):
    ...
    if request.method == 'POST':
        ...
        if admin_add_finance_category_form.is_valid():
            get_user_username = admin_add_finance_category_form.save(
                commit=False)
            get_user_username.save()
            messages.success(
                request, f'{get_user_username.category_name} category was added successfully for {get_user_username.category_owner.username}')
...
def admin_edit_finance_category(request, pk):
    ...
    admin_edit_finance_category_form = AdminFinanceCategoryEditForm(
        instance=qs)
    if request.method == 'POST':
        admin_edit_finance_category_form = AdminFinanceCategoryEditForm(
            request.POST, instance=qs)
        if admin_edit_finance_category_form.is_valid():
            get_user_username = admin_edit_finance_category_form.save(
                commit=False)
            get_user_username.save()
            messages.success(
                request, f'{get_user_username.category_name} category was updated successfully for {get_user_username.category_owner.username}')
...
def admin_edit_task(request, pk):
    ...
    if request.method == 'POST':
        ...
        if admin_edit_task_form.is_valid():
            ...
            get_user_username.save()
            messages.success(
                request, f'task saved for {get_user_username.user.username}')
...
def admin_delete_task(request, pk):
    ...
    if request.method == 'POST':
        qs.delete()
        messages.success(
            request, f'Task for {qs.user.username} was deleted successfully')
        return redirect('homapp:admin_list_and_add_task')
    context = {
        'qs': qs,
    }
...
def admin_add_wear(request):
    ...
    if request.method == 'POST':
        ...
        if admin_add_wear_form.is_valid():
            ...
            messages.success(
                request, f'wear added for {get_username_for_wear.user.username}')
            return redirect('homapp:all_wears_for_admin')
...
def admin_list_and_add_task(request):
    ...
    all_users_that_create_task = Task.objects.values_list(
    ...
    if request.method == 'POST':
        ...
        if admin_add_task_form.is_valid():
            ...
            admin_add_task_form = AdminAddTaskForm()
            messages.success(
                request, f'Task added successfully {get_task_user_username.user.username}'.title())
            return redirect('homapp:admin_list_and_add_task')
...
def all_task_categories_for_admin(request):
    ...
    deactivated_task_categories = TaskCategory.objects.filter(
        activate=False).order_by('-date_created').count()
    ...
    users_that_create_task_categories = TaskCategory.objects.values_list(
        'user', flat=True).distinct()
    ...
    if request.method == 'POST':
        ...
        if add_task_category_form.is_valid():
            ...
            add_task_category_form = TaskCategoryForm()
            messages.success(
                request, f'Task category was created successfully for {add_user.user.username}'.title())
    ...
    # last added task category
    last_added_task_category = TaskCategory.objects.filter(
        activate=True).order_by('-date_created')[:1]
...
def admin_edit_task_category(request, pk):
    ...
    if request.method == 'POST':
        admin_edit_task_category_form = AdminEditTaskCategoryForm(
            request.POST, instance=qs)
        if admin_edit_task_category_form.is_valid():
            get_name = admin_edit_task_category_form.save(commit=False)
            get_name.save()
            messages.success(
                request, f'task category updated successfully for {get_name.user.username}')
            return redirect('homapp:all_task_categories_for_admin')
...
def admin_delete_task_category(request, pk):
    ...
    if request.method == 'POST':
        qs.delete()
        messages.success(
            request, f' Task category was deleted successfully for {qs.user.username}')
        return redirect('homapp:all_task_categories_for_admin')
    context = {
        'qs': qs,
